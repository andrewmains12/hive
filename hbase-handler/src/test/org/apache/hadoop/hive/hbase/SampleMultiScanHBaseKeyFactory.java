 ...
 */
public class SampleMultiScanHBaseKeyFactory extends SampleHBaseKeyFactory2 {
  ...
  public DecomposedPredicate decomposePredicate(JobConf jobConf, Deserializer deserializer, ExprNodeDesc predicate) {
    ...
    try {
      ...
      if (conditionOnBucket) {
        scanRanges = Lists.newArrayList(super.setupFilter(keyMapping.columnName, searchConds));
      } else {
        ...
        for (int i = 0; i < this.numBuckets; i++) {
          ...
          scanRanges.add(super.setupFilter(keyMapping.columnName, searchCondsWithBucket));
        }
      }
    } catch (IOException e) {
